...
export default function RatingChart({ rating = 4.5, totalReviews = 8562 }) {
  const distribution = [
    { stars: 5, count: Math.floor(totalReviews * 0.75), percentage: 75 },
    { stars: 4, count: Math.floor(totalReviews * 0.15), percentage: 15 },
    { stars: 3, count: Math.floor(totalReviews * 0.06), percentage: 6 },
    ...
    <div className="bg-white rounded-xl p-4 shadow-sm">
      <div className="flex items-center gap-4 mb-4">
        <div className="text-center">
          ...
          <div className="text-sm text-gray-500">{totalReviews.toLocaleString()} 条评价</div>
        </div>
        <div className="flex-1 space-y-1.5">
          {distribution.map((item) => (
            <div key={item.stars} className="flex items-center gap-2">
              <span className="text-xs text-gray-500 w-8">{item.stars}星</span>
...
export function PriceRangeChart({ merchants }) {
  const priceRanges = useMemo(() => {
    const ranges = [
      { name: '0-50元', min: 0, max: 50, count: 0, color: '#FF6B00' },
      { name: '50-100元', min: 50, max: 100, count: 0, color: '#FF9F45' },
      { name: '100-200元', min: 100, max: 200, count: 0, color: '#FFB366' },
      { name: '200-500元', min: 200, max: 500, count: 0, color: '#FFD08A' },
      { name: '500元+', min: 500, max: 9999, count: 0, color: '#FFE4BD' },
    ]
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-sm">
      <h3 className="font-bold text-gray-800 mb-4">价格分布</h3>
      <div className="space-y-3">
        {priceRanges.map((range) => (
          <div key={range.name} className="flex items-center gap-3">
            <span className="text-xs text-gray-600 w-16">{range.name}</span>
...
export function PopularityTrend({ data }) {
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-sm">
      <h3 className="font-bold text-gray-800 mb-4">人气趋势 (近7天)</h3>
        ...
        {trendData.map((item, idx) => (
          <div key={idx} className="flex-1 flex flex-col items-center gap-1">
            <div 
              ...
              title={`浏览: ${item.views}, 订单: ${item.orders}`}
            />
            <span className="text-xs text-gray-400">{item.day}日</span>
          </div>
        ))}
      </div>
